...
def thread_find(i, previous_step, step):
    global thread_count
    for n in range(previous_step, step):
        # print('Calculating:', n, end='\r')
        search(n)
    thread_count += 1
    print('Thread:', i, 'Done')
...
def find(num):
    ...
    step = int(num / (core_num - 2))
    previous_step = 0
    for i in range(1, core_num):
        print('Thread:', i, "range:", previous_step, step)

        t = Thread(target=thread_find, args=(i, previous_step, step))
        t.start()

        previous_step = step
        step = (i + 1) * int(num / (core_num - 2))
    while thread_count < (core_num - 1):
        print('', end='\r')
    time.sleep(0.5)
    if thread_count == (core_num-1):
        return None
def clean_up():
...
def control():  # control centre...
    try:
        i = int(input('\nmax limit: '))
        find(i)
        while thread_count<(multiprocessing.cpu_count()-1):
            time.sleep(0.1)
        clean_up()
        print_results()
    except:
